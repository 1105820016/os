         ...
         ;以下进入保护模式... ...
         jmp dword 0x0008:flush             ;16位的描述符选择子：32位偏移  dword修饰偏移量，选择了索引号为1的描述符，从描述符中读取段基址段界限属性放入CS的DCR，偏移量放入EIP
                                            ;清流水线并串行化处理器，jmp指令会自动清流水线
         [bits 32] 							;伪指令，告诉处理器用32为模式译码
    ...
    flush:
         ...
         mov ebp,esp                        ;保存堆栈指针 
         push byte '.'                      ;压入立即数（字节），因为D/B是1，所以压入的是4字节。byte只是告诉编译器操作数是1字节，但是CPU执行时压入的4字节
